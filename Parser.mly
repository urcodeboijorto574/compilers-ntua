...
expr: 
    | T_integer {   
        { expr_kind = E_const($1); expr_type = T_int } 
    }
    | T_chr { 
        { expr_kind = E_char($1); expr_type = T_char } 
    }
    | l_value {
        { expr_kind = E_lvalue($1); expr_type = } (* to be completed *) 
    }
    | T_left_par expr T_right_par {
        { expr_kind = E_expr_parenthesized($2); expr_type = $2.expr_type }
    }
    | func_call {
        { expr_kind = E_func_call($1); expr_type = $1.func_type (* must define type for func_call with type *) }
    }
    | T_plus expr {
        (* must call a semantic function first to check that the type is integer*)
        sem_expr_op $2.expr_type;
        { expr_kind = E_op_expr(O_plus, $2); expr_type = $2.expr_type }
    }
    | T_minus expr {
        (* must call a semantic function first to check that the type is integer*)
        sem_expr_op $2.expr_type;
        { expr_kind = E_op_expr(O_minus, $2); expr_type = $2.expr_type }
    }
    | expr T_plus expr {
        (* must call a semantic function first to check that the type is integer*)
        sem_expr_op_op $1.expr_type $3.expr_type;
        { expr_kind = E_op_expr_expr($1, O_plus, $3); expr_type = $2.expr_type }
    }
    | expr T_minus expr {
        (* must call a semantic function first to check that the type is integer*)
        sem_expr_op_op $1.expr_type $3.expr_type;
        { expr_kind = E_op_expr_expr($1, O_minus, $3); expr_type = $2.expr_type }
    }
    | expr T_mul expr {
        (* must call a semantic function first to check that the type is integer*)
        sem_expr_op_op $1.expr_type $3.expr_type;
        { expr_kind = E_op_expr_expr($1, O_mul, $3); expr_type = $2.expr_type }
    }
    | expr T_div expr {
        (* must call a semantic function first to check that the type is integer*)
        sem_expr_op_op $1.expr_type $3.expr_type;
        { expr_kind = E_op_expr_expr($1, O_div, $3); expr_type = $2.expr_type }
    }
    | expr T_mod expr {
        (* must call a semantic function first to check that the type is integer*)
        sem_expr_op_op $1.expr_type $3.expr_type;
        { expr_kind = E_op_expr_expr($1, O_mod, $3); expr_type = $2.expr_type }
    }
